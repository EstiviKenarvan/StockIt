<?php
// ── Lógica de exportar/importar ANTES de cualquier output HTML ──
if (session_status() === PHP_SESSION_NONE) session_start();

// Credenciales desde env.php (raíz del proyecto)
$_env = require dirname(__DIR__) . '/env.php';
define('DB_HOST', $_env['DB_HOST']);
define('DB_USER', $_env['DB_USER']);
define('DB_PASS', $_env['DB_PASS']);
define('DB_NAME', $_env['DB_NAME']);

function stockit_conectar() {
    $m = new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME);
    $m->set_charset('utf8mb4');
    if ($m->connect_error) return null;
    $m->query("CREATE TABLE IF NOT EXISTS `respaldos_historial` (
...
// ── EXPORTAR ─────────────────────────────────────────────────────
if (isset($_GET['menu']) && $_GET['menu'] === 'respaldo'
    && isset($_GET['submenu']) && $_GET['submenu'] === 'exportar') {

    $m = stockit_conectar();
    if (!$m) {
        $_SESSION['respaldo_error'] = 'No se pudo conectar a la base de datos.';
    ...
    while ($f = $res->fetch_row()) $tablas[] = $f[0];

    $sql  = "-- StockIt | Respaldo SQL\n";
    $sql .= "-- Generado: " . date('Y-m-d H:i:s') . "\n";
    $sql .= "-- Usuario : " . ($_SESSION['usuario'] ?? 'Sistema') . "\n\n";
    ...
    foreach ($tablas as $tabla) {
        ...
        $rc   = $m->query("SHOW CREATE TABLE `$tabla`");
        $fc   = $rc->fetch_row();
        $sql .= $fc[1] . ";\n\n";

        $rd    = $m->query("SELECT * FROM `$tabla`");
        $total = $rd->num_rows;
        if ($total > 0) {
            $cq   = $m->query("SHOW COLUMNS FROM `$tabla`");
            $cols = [];
            while ($c = $cq->fetch_assoc()) $cols[] = '`' . $c['Field'] . '`';
            $cols_str = implode(', ', $cols);
            $lote = []; $cnt = 0;
            while ($fila = $rd->fetch_row()) {
                $vals   = array_map(fn($v) => $v === null ? 'NULL' : "'" . $m->real_escape_string($v) . "'", $fila);
                $lote[] = '(' . implode(', ', $vals) . ')';
                if (++$cnt % 500 === 0) {
            ...
            if (!empty($lote))
                $sql .= "INSERT INTO `$tabla` ($cols_str) VALUES\n" . implode(",\n", $lote) . ";\n";
            $sql .= "\n";
            $notas_rep[] = "Tabla `$tabla`: $total fila(s) exportada(s).";
        } else {
            $notas_rep[] = "Tabla `$tabla`: sin datos.";
        }
    }
    $sql .= "SET FOREIGN_KEY_CHECKS = 1;\n";

    $bytes  = strlen($sql);
    $tamano = $bytes < 1048576 ? round($bytes/1024, 2).' KB' : round($bytes/1048576, 2).' MB';
    $nombre = 'stockit_respaldo_' . date('Ymd_His') . '.sql';

    $u = $m->real_escape_string($_SESSION['usuario'] ?? 'Sistema');
    $n = $m->real_escape_string(implode("\n", $notas_rep));
    $a = $m->real_escape_string($nombre);
    $t = $m->real_escape_string($tamano);
    $m->query("INSERT INTO respaldos_historial (operacion,archivo,tamano,usuario,notas,fecha_hora)
               VALUES ('EXPORTACION','$a','$t','$u','$n',NOW())");
    $m->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST'
    && isset($_GET['menu'])    && $_GET['menu']    === 'respaldo'
    && isset($_GET['submenu']) && $_GET['submenu'] === 'importar') {

    if (empty($_FILES['archivo_respaldo']['tmp_name'])
        || $_FILES['archivo_respaldo']['error'] !== UPLOAD_ERR_OK) {
        $_SESSION['respaldo_error'] = 'No se recibió ningún archivo.';
        header('Location: index.php?menu=respaldo');
        exit;
    }

    $ext = strtolower(pathinfo($_FILES['archivo_respaldo']['name'], PATHINFO_EXTENSION));
    if (!in_array($ext, ['sql', 'txt'])) {
        $_SESSION['respaldo_error'] = 'Solo se aceptan archivos .sql o .txt generados por StockIt.';
        header('Location: index.php?menu=respaldo');
        exit;
    }

    $contenido = file_get_contents($_FILES['archivo_respaldo']['tmp_name']);
    if (!$contenido || trim($contenido) === '') {
        $_SESSION['respaldo_error'] = 'El archivo está vacío.';
        header('Location: index.php?menu=respaldo');
        exit;
    }
    ...
    $errores = []; $ejecutadas = 0;
    $sentencia = ''; $en_str = false; $ch_str = '';
    $len = strlen($contenido);
    for ($i = 0; $i < $len; $i++) {
        $c = $contenido[$i];
        if ($en_str) {
            $sentencia .= $c;
            if ($c === '\\') { $sentencia .= $contenido[++$i] ?? ''; }
            elseif ($c === $ch_str) { $en_str = false; }
            continue;
        }
        if ($c === "'" || $c === '"' || $c === '`') { $en_str = true; $ch_str = $c; $sentencia .= $c; continue; }
        if ($c === '-' && isset($contenido[$i+1]) && $contenido[$i+1] === '-') { while ($i < $len && $contenido[$i] !== "\n") $i++; continue; }
        if ($c === '#') { while ($i < $len && $contenido[$i] !== "\n") $i++; continue; }
        if ($c === '/' && isset($contenido[$i+1]) && $contenido[$i+1] === '*') { $i+=2; while ($i < $len && !($contenido[$i]==='*' && isset($contenido[$i+1]) && $contenido[$i+1]==='/')) $i++; $i+=2; continue; }
        if ($c === ';') {
            $s = trim($sentencia);
            if ($s !== '') {
                if (!$m->query($s)) $errores[] = $m->error . ' → ' . substr($s, 0, 100) . '…';
                else $ejecutadas++;
            }
            $sentencia = ''; continue;
        }
    ...
    $nombre_orig = basename($_FILES['archivo_respaldo']['name']);
    if (empty($errores)) {
        $notas = "Importación exitosa.\nArchivo: $nombre_orig\nSentencias ejecutadas: $ejecutadas\nTamaño: $tamano";
        $_SESSION['respaldo_ok'] = "Respaldo importado correctamente ($ejecutadas sentencias ejecutadas).";
    } else {
        $notas = "Importación con errores.\nEjecutadas: $ejecutadas\nErrores: " . count($errores) . "\n\n" . implode("\n", array_slice($errores, 0, 10));
        $_SESSION['respaldo_error'] = 'Importación con ' . count($errores) . ' error(es). Revisa el reporte.';
    }

    $u = $m->real_escape_string($_SESSION['usuario'] ?? 'Sistema');
    $n = $m->real_escape_string($notas);
    $a = $m->real_escape_string($nombre_orig);
    $t = $m->real_escape_string($tamano);
    $m->query("INSERT INTO respaldos_historial (operacion,archivo,tamano,usuario,notas,fecha_hora)
               VALUES ('IMPORTACION','$a','$t','$u','$n',NOW())");
    $m->close();
...
// ── Cargar historial para la vista ────────────────────────────────
$historial = [];
$m = stockit_conectar();
if ($m) {
    $res = $m->query("SELECT * FROM respaldos_historial ORDER BY fecha_hora DESC");
    if ($res) while ($f = $res->fetch_assoc()) $historial[] = $f;
    $m->close();
}

include dirname(__DIR__) . "/views/includes/tarjeta.php";
include dirname(__DIR__) . "/views/includes/tablas.php";
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<div class="wrapper">
    ...
    <?php $paginaActiva = "respaldo"; include dirname(__DIR__) . "/views/includes/barras.php"; ?>
                ...
                <form action="index.php?menu=respaldo&submenu=importar" method="POST"
                    ...
                    <div class="modal-body">
                        <div class="alert alert-warning">
                            ...
                            <strong>¡Advertencia!</strong> Importar restaurará la base de datos al estado
                            exacto del respaldo. Los datos ingresados después del respaldo serán eliminados.
                        </div>
                        <div class="form-group">
                            <label class="font-weight-bold">
                                Selecciona el archivo <span class="text-stockit">(.sql)</span>
                            </label>
                            <div class="custom-file">
                                <input type="file" class="custom-file-input" id="archivoRespaldo"
                                       name="archivo_respaldo" accept=".sql,.txt" required>
...
$('#btnExportar').on('click', function(e) {
    ...
    Swal.fire({
        ...
        text: 'Se descargará un archivo .sql con todos los datos del sistema.',
        icon: 'question',
        ...
        if (r.isConfirmed) {
            // Descarga el archivo en iframe oculto para no salir de la página
            const iframe = document.createElement('iframe');
            iframe.style.display = 'none';
            iframe.src = url;
            document.body.appendChild(iframe);
            // Recarga la página después de 3s para mostrar el nuevo registro en el historial
